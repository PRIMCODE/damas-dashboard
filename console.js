		...
		function show_users(){
			...
			damas.search_mongo({'username':'REGEX_.*'}, {"username":1},0,0, function(res){
				damas.read(res, function(users){
					...
					th3.classList.add('lastlogin');
					th3.classList.add('time');
					th1.innerHTML = 'username &xutri;';
					th2.innerHTML = 'class';
					th3.innerHTML = 'last login';
					...
					for (var i=0; i<users.length; i++) {
						...
						if (users[i].lastlogin){
							td3.innerHTML= human_time(new Date(users[i].lastlogin));
						}
						tr.appendChild(td1);
						tr.appendChild(td2);
						tr.appendChild(td3);
